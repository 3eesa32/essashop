...
export async function processCoreOrder(orderId, paymentStatus, paymentMethodName = 'فيزا') {
    ...
    // إدخال الفاتورة (تمت إزالة NOW والاعتماد على الداتابيز مباشرة)
    await db.execute(
    ...
    // التعديل هنا: تنسيق التاريخ ليقبله Postgres بدون مشاكل
    const formattedDeadline = deadline.toISOString().split('T')[0]; 

    await db.execute(
        "INSERT INTO projects (id, user_id, invoice_id, title, client_name, deadline, status, progress, notes) VALUES (?, ?, ?, ?, ?, ?, 'pending', 0, ?)",
